...
def is_industry_affiliation(affiliation: str) -> bool:
    """
    Check if an affiliation string contains industry-related keywords.
    """
...
def extract_details(pubmed_id: str, debug: bool = False) -> Optional[Dict[str, str]]:
    """
    Extract details from a PubMed article given its ID.
    ...
    try:
        ...
        if debug:
            logger.warning(f"Missing fields in article {pubmed_id}: {e}")
        return None
